...
class ImageService {
  ...
  Future<File?> pickAndProcessAvatar({required ImageSource source}) async {
    try {
      ...
      // 2. 用户手动裁剪为正方形 (1:1比例)
      final croppedFile = await ImageCropService.cropImageInteractively(pickedFile.path);
      if (croppedFile == null) {
  ...
  Future<File> _compressImage(String imagePath) async {
    try {
      ...
      if (newWidth > maxImageSize || newHeight > maxImageSize) {
        final ratio = maxImageSize / (newWidth > newHeight ? newWidth : newHeight);
      ...
      debugPrint('图片压缩完成: 原大小 ${originalBytes.length} -> 压缩后 ${compressedBytes.length}');
      return compressedFile;
    } catch (e) {
      debugPrint('图片压缩失败: $e');
      // 如果压缩失败，返回原文件
  ...
  /// 生成文件名
  String generateFileName(String studentId, String extension) {
    // `${ramdonNum}-${uuid()}.${extension}`
    final timestamp = DateTime.now().millisecondsSinceEpoch;
    final random = _generateRandom(studentId, timestamp);
    final uuid = _generateUuid();
    return '$random-$uuid.$extension';
  }

  /// 生成随机数
  int _generateRandom(String studentId, int timestamp) {
    // 使用studentId作为种子生成随机数
    final seed = int.tryParse(studentId) ?? timestamp;
    return (timestamp + seed) % 1000000000000000;
  }
  ...
  String _generateUuid() {
    ...
    return md5.convert('$timestamp$random'.codeUnits).toString().substring(0, 8);
  }
