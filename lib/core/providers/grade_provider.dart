...
class GradeNotifier extends StateNotifier<GradeState> {
  ...
  static SemesterInfo _getDefaultSemester() {
    ...
    // 如果没有可用学期，回退到当前学期
    return SemesterInfo(
      xnm: DateTime.now().year.toString(),
      xqm: _getCurrentSemester(),
  ...
  static String _getSemesterDisplayName(String xnm, String xqm) {
    ...
    final season = xqm == '12' ? '春' : xqm == '3' ? '秋' : '夏';
  ...
  Future<void> _loadFromStorage() async {
    try {
      ...
      final gradeSummariesJson = await _secureStorage.read(key: 'gradesSummary');
  ...
  List<CalculatedGrade> _calculateCurrentSemesterGrades(
    ...
    debugPrint('🎓 开始计算学期成绩，目标学期: ${semester.xnm}-${semester.xqm} (${semester.displayName})');
    ...
    for (final summary in gradeSummaries) {
      ...
      if (summary.xnm == semester.xnm && summary.xqm == semester.xqm) {
        ...
        final jd = _calculateGPA(summary.bfzcj, double.tryParse(summary.xf) ?? 0);
    ...
    if (matchedCount == 0 && gradeSummaries.isNotEmpty) {
      ...
      final availableTerms = gradeSummaries.map((s) => '${s.xnm}-${s.xqm}').toSet();
  ...
  double _calculateGPA(String score, double credits) {
    ...
    // 计算绩点
    final gpaRanges = [
      [62, 1.2], [64, 1.4], [66, 1.6], [68, 1.8], [71, 2.0],
      [74, 2.3], [77, 2.6], [80, 3.0], [83, 3.3], [86, 3.6],
      [89, 4.0], [92, 4.3], [95, 4.6], [98, 4.8], [100, 5.0],
    ];
  ...
  /// 排序成绩
  List<CalculatedGrade> _sortGrades(List<CalculatedGrade> grades, GradeSortBy sortBy) {
    ...
    switch (sortBy) {
      ...
      case GradeSortBy.credit:
        sortedGrades.sort((a, b) => 
          double.parse(b.xf).compareTo(double.parse(a.xf)));
        break;
      ...
      case GradeSortBy.gpa:
        sortedGrades.sort((a, b) => 
          double.parse(b.jd).compareTo(double.parse(a.jd)));
        break;
    }
  ...
  GradeStatistics _calculateStatistics(List<GradeSummary> gradeSummaries) {
    ...
    return GradeStatistics(
      compulsoryAverage: totalComCredits > 0 ? totalComGrade / totalComCredits : 0,
