...
class ClassTable {
  ...
  /// 从格式化的JSON创建课表对象
  factory ClassTable.fromFormattedJson(Map<String, dynamic> json, {required String xnm, required String xqm}) {
  ...
  factory ClassTable.fromRawJson(dynamic rawData) {
    ...
    try {
      ...
      if (rawData is String) {
        // 如果是字符串，尝试解析为JSON
        debugPrint('尝试解析JSON字符串: ${rawData.substring(0, min(100, rawData.length))}...');
      ...
      if (data['kbList'] != null) {
        ...
        for (final item in kbList) {
          ...
          try {
            ...
            // 输出调试信息
            debugPrint('解析课程: ${course.title}, 周${course.weekday}, 第${course.periods.join(",")}节, 周次${course.weeks.join(",")}');
            ...
            for (final week in course.weeks) {
              ...
              if (existingCourseIndex != -1) {
                // 有相同课程，合并教师信息
                final existingCourse = weekTable[week]![weekday]![existingCourseIndex];
                if (!existingCourse.teacher.contains(course.teacher)) {
                  ...
                  weekTable[week]![weekday]![existingCourseIndex] = updatedCourse;
                }
              } else {
                // 添加新课程
  ...
  Map<String, dynamic> toJson() {
    ...
    weekTable.forEach((week, weekdayMap) {
      ...
      weekdayMap.forEach((weekday, courses) {
        weekJson['$weekday'] = courses.map((course) => course.toJson()).toList();
      });
    });
