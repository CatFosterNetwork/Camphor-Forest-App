...
class ApiClassTableRepository implements ClassTableRepository {
  ...
  Future<ClassTable> fetchRemote(String xnm, String xqm) async {
    AppLogger.debug('开始从远程获取课表数据 (xnm: $xnm, xqm: $xqm)');

    try {
      // 使用原有的API方法获取数据
      final responseData = await api.fetchClassTable(xnm: xnm, xqm: xqm);
      AppLogger.debug('获取到原始课表数据类型: ${responseData.runtimeType}');

      // 使用增强的fromRawJson方法解析数据
      AppLogger.debug('开始解析课表数据...');
      ...
      AppLogger.debug(
        '解析完成, 总周数: ${table.weekTable.length}, 总课程数: $totalCourses',
      );

      // 如果成功获取到课表信息，先清空本地缓存再保存新数据
      if (totalCourses > 0) {
        AppLogger.debug('成功获取课表数据，先清空本地缓存');
        await clearLocal(xnm, xqm);

        AppLogger.debug('开始保存新的课表数据到本地缓存');
        await saveLocal(xnm, xqm, table);
        AppLogger.debug('新课表数据已保存到本地缓存');
      } else {
        AppLogger.debug('未解析到有效课程，不保存到本地缓存');
      }

      return table;
    } catch (e, stackTrace) {
      AppLogger.debug('获取课表数据时出错: $e');
      AppLogger.debug('错误堆栈: $stackTrace');

      // 尝试从本地加载
      final localData = await loadLocal(xnm, xqm);
      if (localData != null) {
        AppLogger.debug('远程获取失败，使用本地缓存数据');
        return localData;
      }
  ...
  Future<ClassTable?> loadLocal(String xnm, String xqm) async {
    AppLogger.debug('尝试从本地加载课表 (xnm: $xnm, xqm: $xqm)');

    try {
      final str = prefs.getString(_key(xnm, xqm));
      if (str == null || str.isEmpty) {
        AppLogger.debug('本地无缓存数据');
        return null;
      }

      AppLogger.debug('找到本地缓存数据, 长度: ${str.length} 字节');

      // 确保JSON字符串有效
      if (str == '{}' || str == '[]') {
        AppLogger.debug('本地缓存数据为空对象或数组');
        return null;
      }
      ...
      if (json is! Map<String, dynamic>) {
        AppLogger.debug('本地缓存数据格式错误: 不是有效的JSON对象');
        return null;
      }
  ...
  Future<void> saveLocal(String xnm, String xqm, ClassTable data) async {
    try {
      AppLogger.debug('保存课表数据到本地 (xnm: $xnm, xqm: $xqm)');
      final json = data.toJson();
      final jsonStr = jsonEncode(json);
      AppLogger.debug('准备保存的数据大小: ${jsonStr.length} 字节');

      if (jsonStr.length <= 2) {
        AppLogger.debug('警告: 保存的数据可能为空');
      }

      await prefs.setString(_key(xnm, xqm), jsonStr);
      AppLogger.debug('课表数据保存成功');
    } catch (e) {
      AppLogger.debug('保存课表数据失败: $e');
    }
  }
  ...
  Future<void> clearLocal(String xnm, String xqm) async {
    try {
      AppLogger.debug('清空本地课表缓存 (xnm: $xnm, xqm: $xqm)');
      await prefs.remove(_key(xnm, xqm));
      AppLogger.debug('本地课表缓存已清空');
    } catch (e) {
      AppLogger.debug('清空本地课表缓存失败: $e');
    }
  }
}
